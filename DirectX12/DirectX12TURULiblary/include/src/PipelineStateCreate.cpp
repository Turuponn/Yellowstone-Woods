...
void PipelineStateCreate::CreatePipelineStateObj(ID3D12Device* device,ID3D12RootSignature* rootsignature, ID3DBlob* vs, ID3DBlob* ps, D3D12_INPUT_ELEMENT_DESC* inputlaout,const unsigned int inputlayoutelement, ID3D12PipelineState** pipelineobj) {
	HRESULT result = E_FAIL;


	D3D12_GRAPHICS_PIPELINE_STATE_DESC d3d12_gpsd = {};
	d3d12_gpsd.pRootSignature = rootsignature;//ルートシグネチャ
	d3d12_gpsd.BlendState = CD3DX12_BLEND_DESC(D3D12_DEFAULT);
	d3d12_gpsd.VS = CD3DX12_SHADER_BYTECODE(vs);
	d3d12_gpsd.PS = CD3DX12_SHADER_BYTECODE(ps);
	...
	d3d12_gpsd.InputLayout.NumElements = inputlayoutelement;
	d3d12_gpsd.InputLayout.pInputElementDescs = inputlaout;
	...
	result = device->CreateGraphicsPipelineState(&d3d12_gpsd, IID_PPV_ARGS(pipelineobj));
	if (result != S_OK) {
		throw(GameError::GameError(GameErrorNS::FATAL_ERROR, _T("PipelineStateCreate::CreatePipelineStateObj() result != S_OK")));
	}
}
void PipelineStateCreate::CreatePipelineStateObjShadowMap(ID3D12Device* device, ID3D12RootSignature* rootsignature, ID3DBlob* vs, D3D12_INPUT_ELEMENT_DESC* inputlaout, const unsigned int inputlayoutelement, ID3D12PipelineState** pipelineobj) {

	D3D12_GRAPHICS_PIPELINE_STATE_DESC pipeline_state_desc{};
	HRESULT result = E_FAIL;

	//シェーダーの設定
	pipeline_state_desc.VS.pShaderBytecode = CD3DX12_SHADER_BYTECODE(vs).pShaderBytecode;
	pipeline_state_desc.VS.BytecodeLength = CD3DX12_SHADER_BYTECODE(vs).BytecodeLength;


	//インプットレイアウトの設定
	pipeline_state_desc.InputLayout.pInputElementDescs = inputlaout;
	pipeline_state_desc.InputLayout.NumElements = inputlayoutelement;
	...
	//ルートシグネチャ
	pipeline_state_desc.pRootSignature = rootsignature;
	...
	result = device->CreateGraphicsPipelineState(&pipeline_state_desc, IID_PPV_ARGS(pipelineobj));
	if (result != S_OK) {
		throw(1);
	}
}

void PipelineStateCreate::CreatePipelineStateObjShadowMap2(ID3D12Device* device, ID3D12RootSignature* rootsignature, ID3DBlob* vs, ID3DBlob* ps, D3D12_INPUT_ELEMENT_DESC* inputlaout, const unsigned int inputlayoutelement, ID3D12PipelineState** pipelineobj) {
	...
	pipeline_state_desc.VS.pShaderBytecode = CD3DX12_SHADER_BYTECODE(vs).pShaderBytecode;
	pipeline_state_desc.VS.BytecodeLength = CD3DX12_SHADER_BYTECODE(vs).BytecodeLength;
	pipeline_state_desc.PS.pShaderBytecode = CD3DX12_SHADER_BYTECODE(ps).pShaderBytecode;
	pipeline_state_desc.PS.BytecodeLength = CD3DX12_SHADER_BYTECODE(ps).BytecodeLength;
...
void PipelineStateCreate::CreatePipelineStateObj(
	...
	D3D12_GRAPHICS_PIPELINE_STATE_DESC d3d12_gpsd = {};
	d3d12_gpsd.pRootSignature = rootsignature;
	d3d12_gpsd.BlendState = CD3DX12_BLEND_DESC(D3D12_DEFAULT);
	d3d12_gpsd.VS = CD3DX12_SHADER_BYTECODE(vs);
	d3d12_gpsd.PS = CD3DX12_SHADER_BYTECODE(ps);
...
void PipelineStateCreate::CreatePStateObjDeferred(PSTATE& pstate, std::vector<DXGI_FORMAT>& rtv_formats, ID3D12PipelineState** pipelineobj) {
	...
	D3D12_GRAPHICS_PIPELINE_STATE_DESC d3d12_gpsd = {};
	d3d12_gpsd.pRootSignature = pstate.rootsignature;//ルートシグネチャ
	d3d12_gpsd.BlendState = CD3DX12_BLEND_DESC(D3D12_DEFAULT);
	d3d12_gpsd.VS = CD3DX12_SHADER_BYTECODE(pstate.vs);
	d3d12_gpsd.PS = CD3DX12_SHADER_BYTECODE(pstate.ps);
	d3d12_gpsd.GS = CD3DX12_SHADER_BYTECODE(pstate.gs);
	d3d12_gpsd.SampleMask = UINT_MAX;
...
void PipelineStateCreate::CreateComputePipelineState(PSTATE_CS& cstate, ID3D12PipelineState** pipelineobj) {
	...
	desc.CS = CD3DX12_SHADER_BYTECODE(cstate.cs);
	desc.Flags = D3D12_PIPELINE_STATE_FLAG_NONE;
	desc.NodeMask = 0;
	desc.pRootSignature = cstate.rootsignature;
